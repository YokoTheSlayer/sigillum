...
pub(crate) fn sha256_hex(input: &[u8]) -> String {
    ...
    let bit_len = u64::try_from(input.len())
        .expect("input length must fit into u64")
        .checked_mul(8)
        .expect("input bit length must fit into u64");
...
fn compress(state: &mut [u32; 8], chunk: &[u8]) {
    ...
    for (&round_constant, &scheduled_word) in ROUND_CONSTANTS.iter().zip(&schedule) {
        let choice = (e & f) ^ (!e & g);
        ...
        let temporary1 = h
            ...
            .wrapping_add(scheduled_word);
        let temporary2 = big_sigma_0.wrapping_add(majority);
...
mod tests {
    ...
    #[test]
    fn matches_known_multi_block_digest() {
        assert_eq!(
